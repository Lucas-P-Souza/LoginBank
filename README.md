...
are Caesar Cipher and Vigenère Cipher. You can select the desired method through the application's 
user interface, which will be used to encrypt and decrypt passwords.
...
and Vigenère cipher, for encrypting passwords. If you choose one, the other method must be 
...
using either Caesar cipher or Vigenère cipher. Just like at the cryptography, here if you 
choose one, the other method must be commented as in the following examples.
...
opções disponíveis são Cifra de César e Cifra de Vigenère. Você pode selecionar o método desejado 
através da interface do aplicativo, que será utilizado para criptografar e descriptografar senhas.
